...
/**
 * Simple_Google_Rating_Settings Class
 *
 * @class Simple_Google_Rating_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package Simple_Google_Rating
 * @author Unity
 */
final class Simple_Google_Rating_Settings {
	/**
	 * Simple_Google_Rating_Admin The single instance of Simple_Google_Rating_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Simple_Google_Rating_Settings Instance
	 *
	 * Ensures only one instance of Simple_Google_Rating_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Simple_Google_Rating_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'simple-google-rating-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'simple-google-rating-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'simple-google-rating-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Simple_Google_Rating()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'simple-google-rating-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'simple-google-rating-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['setting-fields'] = __( 'Settings', 'simple-google-rating' );
		// $settings_sections['special-fields'] = __( 'Special Fields', 'simple-google-rating' );
		...
		return (array)apply_filters( 'simple-google-rating-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'setting-fields':
				...
				$settings_fields['api-key'] = array(
					'name' => __( 'Google API Key', 'simple-google-rating' ),
					'type' => 'text',
					'default' => '',
					'section' => 'setting-fields',
					'description' => '<a href="https://developers.google.com/places/web-service/get-api-key#get_an_api_key" target="_blank">' . __( 'Get an API Key', 'simple-google-rating' ) . '</a> and enable the Places API.'
				);

				$settings_fields['place-id'] = array(
					'name' => __( 'Google Place ID', 'simple-google-rating' ),
					'type' => 'text',
					'default' => '',
					'section' => 'setting-fields',
					'description' => '<a href="https://developers.google.com/places/place-id" target="_blank">' . __( 'Find the Place ID', 'simple-google-rating' ) . '</a>'
				);
		...
		return (array)apply_filters( 'simple-google-rating-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'simple-google-rating-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'simple-google-rating-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'simple-google-rating-' . $v, array() );
