jQuery(document).ready(function(){
	...
	jQuery('#sptArchiveDailyStats').click(function() {
		var spt_id = jQuery('input#post_ID').val();

		jQuery(this).addClass('button-disabled');
		jQuery('#sptArchiveDailyStatsLoader').show();

		jQuery.post(
			ajaxurl,
			{
				action: 'sptArchiveDailyStats',
				spt_id: spt_id
			},
			function(result) {
				jQuery('#sptArchiveDailyStatsLoader').hide();
				window.location.reload(false);
			}
		);
...
function sptDrawChart(chart_data, chart_id_name) {
	...
	if (chart_data.length && chart_data[0].length == 5)
